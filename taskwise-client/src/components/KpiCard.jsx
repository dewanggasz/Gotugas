...
export default function KpiCard({ user }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm transition-all duration-300 hover:shadow-lg hover:border-blue-200 flex flex-col">
      {/* Bagian Atas: Info Pengguna dan Skor */}
      <div className="flex items-start gap-4">
        <img src={user.profile_photo_url} alt={user.name} className="w-16 h-16 rounded-full object-cover border-2 border-slate-100" />
        <div className="flex-1">
          ...
          <div className={`mt-2 inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-semibold ${level.bg} ${level.color}`}>
        ...
        <div className="text-right">
          ...
          <p className={`text-3xl font-bold ${level.color}`}>{user.kpi_score}</p>
        </div>
      </div>

      {/* Bagian Bawah: Rincian Statistik Tugas */}
      <div className="mt-4 pt-4 border-t border-slate-100 flex justify-around text-center text-xs text-slate-600">
