    ...
    <div className="min-h-screen bg-gray-50 p-8">
      ...
      {!isLoading && !error && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <TaskColumn title="Not Started" tasks={groupedTasks.not_started} summary={getSummary('not_started')} />
          <TaskColumn title="In Progress" tasks={groupedTasks.in_progress} summary={getSummary('in_progress')} />
          <TaskColumn title="Completed" tasks={groupedTasks.completed} summary={getSummary('completed')} />
          <TaskColumn title="Cancelled" tasks={groupedTasks.cancelled} summary={getSummary('cancelled')} />
        </div>
      )}
    </div>
  );
