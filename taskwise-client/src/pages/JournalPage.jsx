...
import MoodPicker from "../components/MoodPicker" // <-- MoodPicker sekarang diimpor dari file terpisah
...
export default function JournalPage() {
  ...
  // Fungsi untuk mengambil semua data (kalender dan detail harian)
  ...
  // Fungsi untuk membuat grid kalender
  const generateCalendarGrid = () => {
    const year = currentDate.getFullYear()
    const month = currentDate.getMonth()
    const firstDayOfMonth = new Date(year, month, 1).getDay()
    const daysInMonth = new Date(year, month + 1, 0).getDate()
    // Menyesuaikan awal minggu ke hari Senin (0 = Senin, 6 = Minggu)
    const startDay = firstDayOfMonth === 0 ? 6 : firstDayOfMonth - 1
    const grid = []
    
    // Sel kosong untuk awal bulan
    for (let i = 0; i < startDay; i++) {
      grid.push({ key: `pad-${i}`, type: "padding" })
    }

    // Sel untuk setiap hari dalam bulan
  ...
  const handleMoodUpdate = async (mood) => {
    // Jika mood yang sama diklik lagi, set mood menjadi null (menghapus mood)
    ...
    try {
      ...
      // Perbarui state secara optimis untuk responsivitas UI
      setDayDetails((prev) => ({ ...prev, mood: response.data.mood }))
      
      // Perbarui data bulan untuk menampilkan indikator mood di kalender jika ada
      fetchAllData(currentDate, selectedDate)
    } catch (error) {
  ...
  const handleNoteSubmit = async (noteData) => {
    try {
      ...
      // Muat ulang data untuk menampilkan catatan baru/yang diperbarui
      fetchAllData(currentDate, selectedDate)
    } catch (error) {
      console.error("Gagal menyimpan catatan:", error)
      throw error // Lempar error agar bisa ditangani di form
    }
  }
  ...
  // Render komponen
  return (
    // Container utama dengan tinggi maksimal 100dvh di layar xl (desktop)
    <div className="min-h-screen xl:h-[100dvh] bg-gradient-to-br from-blue-50 via-white to-indigo-50 p-4 md:p-6 xl:overflow-hidden">
              ...
              <>
                <div className="p-6 space-y-6">
                  <div className="bg-gray-50 p-4 rounded-xl">
                    ...
                    {/* Menggunakan komponen MoodPicker yang sudah diimpor */}
                    <MoodPicker selectedMood={dayDetails?.mood} onSelectMood={handleMoodUpdate} />
                  </div>
                  <button onClick={() => { setEditingNote(null); setIsNoteModalOpen(true); }} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-4 rounded-xl transition-colors duration-200 flex items-center justify-center gap-2">
                  ...
                  <div className="h-full xl:overflow-y-auto space-y-3 pr-2">
                    {dayDetails?.notes?.length > 0 ? (
                      dayDetails.notes.map((note) => (
                        <div key={note.id} className="group bg-white border border-gray-200 p-4 rounded-xl hover:shadow-md transition-all duration-200">
                          <div className="flex justify-between items-start gap-3">
                            <p className="text-gray-700 text-sm leading-relaxed whitespace-pre-wrap flex-1">{note.content}</p>
                      ...
                      <div className="text-center py-12">
                        <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4"><Book className="w-6 h-6 text-gray-400" /></div>
            ...
            <div className="p-6 border-b border-gray-100">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-xl font-bold text-gray-800">{currentDate.toLocaleDateString("id-ID", { month: "long", year: "numeric" })}</h2>
                </div>
            ...
            <div className="flex-1 p-6 overflow-hidden xl:min-h-0">
              <div className="h-full flex flex-col">
                <div className="grid grid-cols-7 gap-1 mb-4">
                  {["Sen", "Sel", "Rab", "Kam", "Jum", "Sab", "Min"].map((day) => (<div key={day} className="text-center text-sm font-semibold text-gray-600 py-2">{day}</div>))}
                </div>
      ...
      <JournalNoteForm isOpen={isNoteModalOpen} onClose={() => setIsNoteModalOpen(false)} onSubmit={handleNoteSubmit} initialNote={editingNote} />
    </div>
  )
}
